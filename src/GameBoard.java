...
implements ActionListener, KeyListener 
{
	...
	public GameBoard(StarFighter game)
	{
		...
		new EasySound("Explosion.wav");
		laserGun = new EasySound("LaserGun.wav");

		SpaceShip = new SpaceShip();
		asteroidArray = new ArrayList<Asteroid>(0);
		coinArray = new ArrayList<Coin>(0);
	...
	public void spawnObject(int num){
		...
		if(x == 1){
			coin = new Coin();
			int r = (int)(Math.random()*(240-coin.getEntityWidth()));
			...
			int r = (int)(Math.random()*(240-asteroid.getEntityWidth()));
	...
	public void drawAsteroids(Graphics g){
		for(int i = 0; i < asteroidArray.size(); i++){
			asteroidArray.get(i).draw(g,1,1);

		}
	}
	public void drawCoins(Graphics g){
		for(int i = 0; i < coinArray.size(); i++){
			coinArray.get(i).draw(g,1,1);
